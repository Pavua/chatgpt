...
- Minimal Electron GUI for adjusting settings and browsing history.
...
edit the current prompt and browse recent history.
...
To produce standalone desktop packages from the GUI directory you can use
`electron-packager`:

```bash
npx electron-packager . userbot --platform=darwin --overwrite
```
